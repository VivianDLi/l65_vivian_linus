...
from torch_geometric.typing import EdgeType, NodeType, OptPairTensor
...
class HeteroConv(torch.nn.Module):
    ...
    def forward(
        ...
        *args_dict,
        **kwargs_dict,
    ) -> Union[Dict[NodeType, Tensor], Tuple[Dict[NodeType, Tensor]]]:
        ...
        for key, value in out_dict.items():
            if use_pos:
                x_value, pos_value = map(list, zip(*value))
                out_dict[key] = group(x_value, self.aggr)
                pos_dict[key] = group(pos_value, self.aggr)
            else:
                out_dict[key] = group(value, self.aggr)

        if use_pos:
            return out_dict, pos_dict
        else:
            return out_dict
